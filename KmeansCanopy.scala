...
object KmeansCanopy {
  def main(args: Array[String]) {
    val conf = new SparkConf().setAppName("KmeansCanopy")
    ...
    //canopy initial centroids
    
    var tfidfcopy = tfidf
	var canopyCentroid = tfidfcopy.takeSample(false,1)
var centroids = {
while(tfidfcopy.count() > 4.toLong){
var canopyCentroid = tfidfcopy.takeSample(false,1)
var newdist = tfidfcopy.map{ line => {
     Vectors.sqdist(line, canopyCentroid(0))
      }
      }
      
   var mean = newdist.mean()
var newValue = tfidfcopy.flatMap{ line => {
      var dist = Vectors.sqdist(line, canopyCentroid(0))
      if(dist<mean && dist > 0.5)
      Some (line)
      else None
      }
      }

    tfidfcopy =  tfidfcopy.subtract(newValue)
